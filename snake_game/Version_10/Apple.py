...
from snake_game.version_02.Configurations import configurations
...
class Apple(Sprite):
    ...
    def __init__(self):
        ...
        self._last_uptade_time=pygame.time.get_ticks()
        self._frame_index=1
    ...
    def animate_apple(self)->None:
        ...
        needs_refresh=(current_time - self._last_uptade_time)>=time_to_refresh

        if needs_refresh:
            self.image = self._apple_frames[self._frame_index]

            self._last_uptade_time = current_time
            self._frame_index += 1
